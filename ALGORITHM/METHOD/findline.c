...
void flMethod_slowest()
{
	...
	if(findLineFlag == 0)//保证每个路段初始化一次
	{
		PID_Init(&glsensorPID,20,2000,300,0,200);																					//对速度PID的参数进行初始化设置
...
void flMethod_sword()
{
	...
	if(findLineFlag == 0)//保证每个路段初始化一次
	{
		PID_Init(&glsensorPID,20,1600,100,0,0);																					//对速度PID的参数进行初始化设置
		speedRec_Init(&glmotorSpeed,1310,1400); 																				//对初始速度进行设定
		findLineFlag=1;
	}
	glsensor_dig_value = sensorAD(glsensor_ad_value,basic_sensorThreshold);  				//与阈值比较后将模拟量转化成数字量
...
void flMethod_NFL()
{
	...
	if(findLineFlag == 0)//保证每个路段初始化一次
	{
		PID_Init(&glsensorPID,16,2600,200,0,200);																					//对速度PID的参数进行初始化设置
		speedRec_Init(&glmotorSpeed,2200,2400); 																				//对初始速度进行设定
		findLineFlag=1;
