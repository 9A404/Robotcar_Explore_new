...
void flMethod_quick()
{
	...
	if(findLineFlag == 0)//保证每个路段初始化一次
	{
			...
			speedRec_Init(&glmotorSpeed,4940,5000); 																				//对初始速度进行设定
		#endif
...
void flMethod_quickest()
{
	...
	if(findLineFlag == 0)//保证每个路段初始化一次
	{
		//条件编译选择调试用低速度
			...
			speedRec_Init(&glmotorSpeed,6450,6500); 																				//对初始速度进行设定
		#endif
...
void flMethod_brige()
{
	...
	if(findLineFlag == 0)   //保证每个路段初始化一次
	{
		...
		speedRec_Init(&glmotorSpeed,2420,2500); 																				//对初始速度进行设定
		findLineFlag=1;
	}
...
void flMethod_upPeak()
{
	...
	if(findLineFlag == 0)//保证每个路段初始化一次
	{
		...
		speedRec_Init(&glmotorSpeed,2890,3000); 																				//对初始速度进行设定
		findLineFlag=1;
	}
	glsensor_dig_value = sensorAD(glsensor_ad_value,basic_sensorThreshold);  				//与阈值比较后将模拟量转化成数字量
	rank=sensorDigitalRank(glsensor_dig_value);                                    //分级
...
void flMethod_right()
{
	...
	if(findLineFlag == 0)//保证每个路段初始化一次
	{
		PID_Init(&glsensorPID,20,3500,120,0,0);																					//对速度PID的参数进行初始化设置
		speedRec_Init(&glmotorSpeed,2080,1000); 																				//对初始速度进行设定2200+200
		findLineFlag=1;
	}
	glsensor_dig_value = sensorAD(glsensor_ad_value,basic_sensorThreshold);  				//与阈值比较后将模拟量转化成数字量
